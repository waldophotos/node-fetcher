/**
 * @fileOverview Fetcher initialization methods.
 */
...
/**
 * Fetcher initialization methods.
 *
 * @constructor
 */
...
FetcherInit.prototype._initKafka = Promise.method(function () {
  ...
  const connectOpts = {
    ...
    onMessage: (message) => {
      this._processMessage(message.value)
        .catch((err) => {
          this.opts.log.error('Fetcher._initKafka() :: Processing error for',
            'topic:', this.opts.topic, 'Will suppress. Error:', err);
        });
    },
    onError: (error) => {
      this.opts.log.error('Fetcher._initKafka() :: Streaming error for topic:',
  ...
  return this.consumer.connect(connectOpts)
    .then(() => {
      this.opts.log.info('Fetcher._initKafka() :: Connected to topic:', this.opts.topic);
    });
});
