...
MessageProcess.prototype._produceKafkaErrorMessage = Promise.method(function (error) {
  if (!this.fetchers.opts.produceErrorMessage) {
    return;
  }

  const producer = new kafkaLib.Producer({
    topic: this.fetchers.opts.topicProduceError,
    schema: this.fetchers.opts.schemaProduceError,
    keySchema: { type: 'string' },
    log: this.fetchers.opts.log,
  });

  const newMessage = this.fetchers.opts.generateErrorMessage(error);
