    ...
    if (response && response[this.fetcher.opts.keyAttribute]) {
      ...
      if (this.fetcher.opts.keyAttribute) {
        this.fetcher.opts.log.warn('Fetcher.MessageProcess._produceKafkaMessage() ::',
          ...
          this.fetcher.opts.topicProduce, 'Key to use:', this.fetcher.opts.keyAttribute,
          'Response:', response);
      }
...
MessageProcess.prototype._produceKafkaErrorMessage = Promise.method(function (error) {
  ...
  if (this.fetcher.opts.useKeys) {
    if (newMessage && newMessage[this.fetcher.opts.keyAttributeError]) {

      const key = newMessage[this.fetcher.opts.keyAttributeError];
      ...
      if (this.fetcher.opts.keyAttribute) {
        this.fetcher.opts.log.warn('Fetcher.MessageProcess._produceKafkaErrorMessage() ::',
          '"keyAttributeError" was defined but generator response did not contain it.',
          'Will produce message without key for partionioning on topic:',
          this.fetcher.opts.topicProduceError, 'Key to use:',
          this.fetcher.opts.keyAttributeError, 'Error Message to produce:',
          newMessage);
      }
